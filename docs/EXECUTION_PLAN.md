...
- [x] **#33 Fix residual order-dependence uncovered by #32** — **budget cut 3 days → 1–2 days.** The dominant known cause (unsorted `sources` lists) is fixed by #66, and #32-PRE was green before this milestone started, so anything found here is caused by the assertion reader and is attributable. `S:S`
  - **Reopened and closed again during M10.** The M4 fix canonicalised the entity-derived containers and the snapshot `sources` lists, and was closed on tests that permute *results*. Permuting results never disturbs the order of facts *within* one result, so containers appended one entry per fact — `governance.risk_factors` (keyed on `period` alone) and `strategy.entries` (keyed on `source_date` alone) — kept analyzer emission order and nothing could see it. The assertion reader emits in content-address order, so the two tiers disagreed. Surfaced by TCS's #59 backfill: 184 differences, every one a pure permutation. Fixed by making every non-total key run to the full field set; regression test reverses the facts within a single result, which is the smallest faithful model of the tier divergence.
...
### Found during M10, deferred — neither blocks the milestone

- [ ] **#79 `atlas store status` must be observational and never create `assertions.db`** — running it against a repository with no store *creates* one: it reports `assertions.db: 45,056 bytes` for a repository that had none a moment earlier, because `AssertionStore(root)` creates the database on open (discovery 26). Observed on TCS during #59, where it silently converted "this repository has never been analyzed" into "this repository has an empty store" — the exact confusion `_existing_runs()` (`migrate.py:266`) already guards against by checking `(root / DB_FILENAME).exists()` first. `store status` needs the same guard, and reporting a store that does not exist should say so rather than manufacture one. A command whose name promises a read must not write. `S:XS`
- [ ] **#81 One `rebuild --from evidence` run took 14,323 s for work that takes ~1,000 s, and did not reproduce** — TCS, 140 documents: 14,323 s (3 h 58 m) on the first run, **1,025 s on the second**, against `migrate assertions`' 734 s and 985 s for the same corpus. Confirmed across the full rollout: SBIN rebuild 4,132 s then migrate 15,422 s for one corpus; TATASTEEL rebuild 691 s then migrate 690 s, and a `--verify` that reads the store and touches no PDF took **10,808 s** where the same command on TCS took 2 s. A ~20x spread on identical work, in both directions, on every command — all 156 `parsed_documents` rows agree on `extraction_method`, `quality_score`, `ocr_attempted`, `char_count` and `status` after every run. Not a systematic slowdown, so the first reading of this as "rebuild is 20x slower than migrate" was wrong: both commands run the same `load_results(source="analyzers")` and both have now been fast and slow. Environmental until shown otherwise (OCR on the 9 Tesseract documents is the obvious suspect for variance). Worth a timing instrument before migrating SBIN (307 docs) and TATASTEEL (287), because the spread — not the mean — is what makes those runs unschedulable. `S:S`
- [ ] **#83 Should the assertion reader restore analyzer emission order?** `Assertion.ordinal` records it (`writer.py` assigns it over `result.facts` in emission order) and the reader discards it, ordering by `(source_date, evidence_id, assertion_id)` — a content hash. Where a container merges by key rather than appending, that makes the surviving value tier-dependent. **Opened as a correctness bug and downgraded to robustness**: the one case that fired (TATASTEEL `financial_fcf`) was an analyzer defect, fixed at the analyzer in `2bdaae0`, and a corpus sweep found exactly one such collision in 734 documents with duplicate emission into a dict-merged container intentional nowhere. So there is currently nothing for emission order to disambiguate. Restoring it would be defence in depth against the next analyzer that emits duplicates; it would also have masked this defect by making both tiers agree on a wrong number. Decide deliberately, not under incident pressure. `S:S`
- [ ] **#82 `rebuild --from evidence` is already a backfill path, which overlaps #59** — it writes every analyzed result into the assertion store (`rebuild.py:161-168`) before building the profile. So the TCS rebuild populated `assertions.db` with all 140 runs, and the `migrate assertions` that followed reported `Already held: 140 run(s) — this is a re-run` and was a content-identical no-op replacement. Nothing is wrong: content addresses make the two stores equivalent, and the migration still verified and moved atomically. But "migrate the existing repositories" and "rebuild the existing repositories" are not distinct operations at Tier 1, and the docs treat them as if they were. Decide which is the supported backfill and say so, or `migrate assertions` becomes a command whose only distinct guarantee is the verification gate. `S:XS`
- [ ] **#80 Investigate whether migrate verification should distinguish a stale Tier 2 artifact from a Tier 1 round-trip failure** — `_verify` (`migrate.py:222`) compares the assertion-path profile against the profile *on disk*, so two unrelated failures produce the same refusal: (a) the backfill lost information, which must refuse, and (b) `profile.json` predates the evidence corpus, which is a Tier 2 staleness problem that `atlas rebuild --from evidence` fixes. TCS refused for (b): 1,661 differences, but the candidate covered 140 evidence ids against the stored profile's 122, a strict superset with zero documents lost, because 42 of 156 documents were first parsed ~20 hours after the profile was written. `migrate_assertions` already holds `report.results` — the analyzer-path results for the current corpus — and discards them, so the like-with-like comparison (M3 #26's invariant) is available at no extra cost. **Any change here must not weaken the gate**: the refusal was correct and the information it printed was sufficient to diagnose the cause in one pass. The question is only whether the two cases should be *named* differently, not whether either should stop refusing. `S:S`
