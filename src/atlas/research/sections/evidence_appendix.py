...
def build(
    ...
    if not seen:
        ...
        notes.append(f"{len(seen)} distinct evidence_id(s) cited; repository not available to resolve citations.")
    else:
        for eid in seen:
            entry = repo.get(eid)
            if entry is None:
            ...
            findings.append(Finding(text=citation.citation_full.replace("\n", " — "), kind="fact"))
