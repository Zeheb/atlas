...
def _margin_stability(
    profile: CompanyProfile, repo: Repository | None
) -> tuple[Finding | None, engine.TableSection | None]:
        ...
        for key in _MARGIN_CANDIDATES:
            ...
            snaps = sorted(
                (
                    ...
                    if s.basis == "consolidated" and s.period_type == period_type
                ),
            ...
            finding = Finding(
                text=(
                    ...
                    + (
                        "stable"
                        if spread < 3
                        else "moderately variable" if spread < 8 else "wide-ranging"
                    )
                    + " over the period."
                ),
                kind=DERIVED,
...
def _growth_consistency(profile: CompanyProfile) -> Finding | None:
    ...
    points = [
        (s.period, s.facts[FactKind.FINANCIAL_REVENUE])
        for s in snaps
        if FactKind.FINANCIAL_REVENUE in s.facts
    ]
    if len(points) < 3:
        return None
    yoy_positive = sum(
        1 for (_, prev), (_, curr) in zip(points, points[1:]) if curr > prev
    )
    ...
    cagr = (
        ((last / first) ** (1 / years) - 1) * 100 if first > 0 and years > 0 else None
    )
