...
class FactKind(enum.Enum):
    ...
    FINANCIAL_COST_OF_MATERIALS = (
        "financial_cost_of_materials"  # "Cost of materials consumed"
    )
    FINANCIAL_PURCHASES_STOCK_IN_TRADE = (
        "financial_purchases_stock_in_trade"  # "Purchases of stock-in-trade"
    )
    ...
    CAPITAL_BUYBACK_PRICE_PER_SHARE = (
        "capital_buyback_price_per_share"  # unit=RUPEES_PER_SHARE
    )
    CAPITAL_BUYBACK_SHARES_OFFERED = (
        "capital_buyback_shares_offered"  # max shares targeted; unit=COUNT
    )
    CAPITAL_BUYBACK_RECORD_DATE = (
        "capital_buyback_record_date"  # eligibility cut-off; unit=ISO_DATE
    )
    CAPITAL_BUYBACK_SHARES_BOUGHT = (
        "capital_buyback_shares_bought"  # shares actually extinguished; unit=COUNT
    )

    # M&A and subsidiary events (capital deployment)
    CAPITAL_ACQ_TARGET_NAME = (
        "capital_acq_target_name"  # entity name as stated in filing; unit=None
    )
    CAPITAL_ACQ_CONSIDERATION_TYPE = (
        "capital_acq_consideration_type"  # "cash"|"share_swap"|"subscription"
    )
    ...
    CAPITAL_ACQ_EXPECTED_COMPLETION = (
        "capital_acq_expected_completion"  # ISO date; unit=ISO_DATE
    )

    # Subsidiary investment / external fundraise events
    CAPITAL_INVEST_TARGET_NAME = (
        "capital_invest_target_name"  # subsidiary receiving the investment; unit=None
    )
    CAPITAL_INVEST_AMOUNT = (
        "capital_invest_amount"  # committed capital; unit=CRORE_INR|USD_BILLION
    )
    ...
    CAPITAL_FUNDRAISE_AMOUNT = (
        "capital_fundraise_amount"  # max amount authorised; unit=CRORE_INR
    )
    ...
    STRATEGY_ASPIRATION = (
        "strategy_aspiration"  # company-level strategic vision; unit=None
    )
    ...
    AUDIT_KAM_TITLE = (
        "audit_kam_title"  # Key Audit Matter title text; unit=None; textual
    )
    ...
    GOVERNANCE_DIRECTOR_CHANGE_ROLE = (
        "governance_director_change_role"  # role string; unit=None
    )

    # AGM voting results (one fact per resolution; period=AGM date; section="resolution_N")
    GOVERNANCE_RESOLUTION_TITLE = (
        "governance_resolution_title"  # agenda text; unit=None
    )
    GOVERNANCE_RESOLUTION_TYPE = (
        "governance_resolution_type"  # "ordinary" | "special"; unit=None
    )
    GOVERNANCE_RESOLUTION_OUTCOME = (
        "governance_resolution_outcome"  # "passed" | "not_passed"; unit=None
    )
    GOVERNANCE_VOTE_PCT_FOR = (
        "governance_vote_pct_for"  # % votes in favour on polled; unit=PERCENT
    )
    GOVERNANCE_VOTE_PCT_AGAINST = (
        "governance_vote_pct_against"  # % votes against on polled; unit=PERCENT
    )
    ...
    GOVERNANCE_RPT_CATEGORY = (
        "governance_rpt_category"  # the disclosed line's own label text; unit=None
    )
    ...
    OWNERSHIP_PROMOTER_PLEDGED_PCT = (
        "ownership_promoter_pledged_pct"  # % of promoter shares pledged
    )
    OWNERSHIP_FPI_PCT = (
        "ownership_fpi_pct"  # foreign portfolio investors (all categories)
    )
    ...
    CREDIT_AMOUNT = (
        "credit_amount"  # unit=CRORE_INR; rated ceiling — absent for ESG / unsolicited
    )
    ...
    FINANCIAL_OPERATING_MARGIN = (
        "financial_operating_margin"  # operating margin %; unit=PERCENT
    )
    FINANCIAL_NET_MARGIN = "financial_net_margin"  # net income margin %; unit=PERCENT
    FINANCIAL_ROE = (
        "financial_roe"  # return on equity; unit=PERCENT; period=fiscal year end
    )
    ...
    FINANCIAL_CAPEX = (
        "financial_capex"  # PP&E purchases only; unit=CRORE_INR; absolute value
    )
    ...
    FINANCIAL_DEBT_MATURITY_WITHIN_1Y = (
        "financial_debt_maturity_within_1y"  # not later than one year or on demand
    )
    ...
    FINANCIAL_NET_INTEREST_MARGIN = (
        "financial_net_interest_margin"  # NIM = NII / avg earning assets; unit=PERCENT
    )
    FINANCIAL_GROSS_NPA_RATIO = (
        "financial_gross_npa_ratio"  # Gross NPA / Gross Advances; unit=PERCENT
    )
    FINANCIAL_NET_NPA_RATIO = (
        "financial_net_npa_ratio"  # Net NPA / Net Advances; unit=PERCENT
    )
    FINANCIAL_PROVISION_COVERAGE_RATIO = "financial_provision_coverage_ratio"  # provisions held / gross NPA; unit=PERCENT
    FINANCIAL_CREDIT_COST = (
        "financial_credit_cost"  # provisioning expense / avg advances; unit=PERCENT
    )
    ...
    ESG_ENERGY_TOTAL_MJ = (
        "esg_energy_total_mj"  # Total energy consumption; unit=MEGAJOULE
    )
    ESG_ENERGY_RENEWABLE_PCT = (
        "esg_energy_renewable_pct"  # Renewable fraction of total energy; unit=PERCENT
    )
    ESG_WATER_CONSUMED_KL = (
        "esg_water_consumed_kl"  # Total water consumed; unit=KILOLITRE
    )
    ESG_WASTE_GENERATED_MT = (
        "esg_waste_generated_mt"  # Total waste generated; unit=METRIC_TONNE
    )
    ESG_WASTE_RECOVERY_PCT = (
        "esg_waste_recovery_pct"  # Waste recovered (recycled + reused) %; unit=PERCENT
    )
    ESG_WORKFORCE_HEADCOUNT = (
        "esg_workforce_headcount"  # Total employees (permanent + other); unit=COUNT
    )
    ESG_WORKFORCE_FEMALE_PCT = (
        "esg_workforce_female_pct"  # Female employees as % of total; unit=PERCENT
    )
...
class FactUnit(enum.Enum):
    ...
    MILLION_TONNES = (
        "million_tonnes"  # bulk production/delivery volume (steel, cement, etc.)
    )
