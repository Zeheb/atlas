...
def _case() -> EvalCase:
    return EvalCase(
        ...
        rubric="question-conditioned retrieval",
    )
...
def test_other_capabilities_present_without_question_retrieval_no_effect(
    tmp_path: Path,
) -> None:
    _seed(tmp_path)
    settings = Settings(_env_file=None, repository_base_path=tmp_path)
    runner = LiveReasoningRunner(
        settings,
        _fake_client(),
        capabilities=frozenset({"single_name", "drilldown"}),
    )
    context = runner.run(_case()).context
    assert not any(c.statement.startswith("Source passage:") for c in context.claims)


def test_no_bundled_case_requires_question_retrieval_it_is_a_runner_mode_switch() -> (
    None
):
