...
def _entry(
    evidence_id: str, source_date: str, acquired_at: str = "2026-04-01T00:00:00+00:00"
) -> CatalogEntry:
    return CatalogEntry(
...
def _seed_catalog_and_kb(base, entries: list[CatalogEntry]) -> None:
    ...
    for entry in entries:
        catalog.add(entry)
        (repo_root / entry.local_path).write_text(
            "Some parsed content here.", encoding="utf-8"
        )
        kb.parse(entry)
    catalog.save()
...
def _view(
    *evidence_ids: str,
    ...
    return RecalledView(
        ...
        claims=(
            RecalledClaim(
                statement="Margins are durable.",
                evidence_ids=frozenset(ids),
                confidence="medium",
            ),
        ),
        as_of=as_of,
...
def test_new_evidence_after_as_of_is_reported(tmp_path) -> None:
    ...
    report = check_staleness(
        _view("bse-ev-1", as_of="2026-06-01T00:00:00+00:00"), tmp_path / "TCS"
    )
...
def test_policy_kinds_filter_restricts_new_evidence(tmp_path) -> None:
    ...
    report = check_staleness(
        _view("bse-ev-1", as_of="2026-06-01T00:00:00+00:00"),
        tmp_path / "TCS",
        policy,
    )
    assert report.new_evidence_since == ()
...
def test_sweep_staleness_returns_empty_for_a_subject_with_no_stored_views(
    tmp_path,
) -> None:
...
def test_sweep_staleness_checks_every_stored_view(tmp_path) -> None:
    ...
    investigation = Investigation(
        ...
        question="What does business quality show?",
        subjects=("TCS",),
        rationale="a required dimension",
        priority=5,
    )
            ...
            return json.dumps(
                {
                    ...
                    "findings": [
                        {
                            "statement": "Durable business.",
                            "assertability": "judgment",
                            "confidence": "medium",
                            "supporting_evidence_ids": ["bse-ev-1"],
                            "known_unknowns": [],
                        }
                    ],
                }
