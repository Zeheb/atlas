...
def _retrieval(selected: tuple[tuple[str, int, int], ...], **overrides: object) -> RetrievalCaseMetrics:
...
def _planner(intent: str, rules: tuple[str, ...]) -> PlannerCaseMetrics:
    return PlannerCaseMetrics(intent=intent, preferred_kinds=(), top_k=5, periods_found=(), rules_fired=rules)
...
def test_disjoint_selection_has_zero_overlap_and_changed_top1() -> None:
    baseline = _report("base", (_case("t01", retrieval_metrics=_retrieval((("ev-1", 10, 250),))),))
    candidate = _report("cand", (_case("t01", retrieval_metrics=_retrieval((("ev-2", 20, 150),))),))
    rc = ranking_change(baseline, candidate)
...
def test_reordering_without_membership_change_shows_displacement_not_churn() -> None:
    ...
    candidate = _report("cand", (_case("t01", retrieval_metrics=_retrieval(
        (("ev-2", 20, 260), ("ev-1", 10, 150)),  # same members, swapped order
    )),))
    rc = ranking_change(baseline, candidate)
    pc = rc["per_case"][0]
...
def test_case_missing_retrieval_metrics_on_either_side_excluded() -> None:
    ...
    candidate = _report("cand", (_case("t01", retrieval_metrics=_retrieval((("ev-1", 0, 100),))),))
    rc = ranking_change(baseline, candidate)
...
def test_retrieval_deltas_computes_expected_arithmetic() -> None:
    baseline = _report("base", (_case("t01", retrieval_metrics=_retrieval(
        (("ev-1", 0, 100),), candidates_considered=10, metadata_coverage=0.5, boost_share=0.1,
    )),))
    candidate = _report("cand", (_case("t01", retrieval_metrics=_retrieval(
        (("ev-1", 0, 100),), candidates_considered=20, metadata_coverage=1.0, boost_share=0.4,
    )),))
...
def _quality(precision: float, recall: float, mrr: float) -> RetrievalQualityScore:
    return RetrievalQualityScore(precision_at_k=precision, recall_at_k=recall, mrr=mrr, forbidden_retrieved=())


def test_retrieval_quality_deltas_computes_expected_arithmetic() -> None:
    baseline = _report("base", (_case("t01", retrieval_quality=_quality(0.5, 0.5, 0.5)),))
    candidate = _report("cand", (_case("t01", retrieval_quality=_quality(1.0, 0.75, 1.0)),))
...
def test_compare_retrieval_includes_retrieval_quality_deltas() -> None:
    baseline = _report("base", (_case("t01", retrieval_quality=_quality(0.5, 0.5, 0.5)),))
    candidate = _report("cand", (_case("t01", retrieval_quality=_quality(1.0, 1.0, 1.0)),))
...
def test_planner_attribution_by_intent_and_rule() -> None:
    baseline = _report("base", (
        _case("t01", correctness_pass=False, grounding_pass=True,
              planner_metrics=_planner("narrative", ("intent_keyword_match",))),
        _case("t02", correctness_pass=True, grounding_pass=True,
              planner_metrics=_planner("general", ("intent_fallback",))),
    ))
    candidate = _report("cand", (
        _case("t01", correctness_pass=True, grounding_pass=True,
              planner_metrics=_planner("narrative", ("intent_keyword_match",))),
        _case("t02", correctness_pass=True, grounding_pass=True,
              planner_metrics=_planner("general", ("intent_fallback",))),
    ))
    ...
    assert attribution["by_rule"]["intent_keyword_match"]["delta_correctness_pass_rate"] == 1.0
...
def test_planner_attribution_excludes_cases_without_planner_metrics() -> None:
    ...
    candidate = _report("cand", (_case("t01", correctness_pass=True),))  # no planner_metrics
    attribution = planner_attribution(baseline, candidate)
...
def test_side_by_side_carries_answer_text_and_scores() -> None:
    baseline = _report("base", (_case(
        "t01", refused=False, correctness_pass=True, grounding_pass=True,
        answer_prose="Margins have been stable.",
        retrieval_metrics=_retrieval((("ev-1", 0, 100),)),
    ),))
    candidate = _report("cand", (_case(
        "t01", refused=False, correctness_pass=True, grounding_pass=True,
...
def test_compare_retrieval_composes_all_sections() -> None:
    baseline = _report("base", (_case(
        "t01", correctness_pass=True, grounding_pass=True, refused=False,
        retrieval_metrics=_retrieval((("ev-1", 0, 100),)),
        planner_metrics=_planner("narrative", ("intent_keyword_match",)),
    ),))
    candidate = _report("cand", (_case(
...
def test_compare_retrieval_is_pure() -> None:
    baseline = _report("base", (_case("t01", retrieval_metrics=_retrieval((("ev-1", 0, 100),))),))
    candidate = _report("cand", (_case("t01", retrieval_metrics=_retrieval((("ev-2", 0, 200),))),))
