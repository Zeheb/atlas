...
def test_coverage_json_format_is_valid_json(monkeypatch, tmp_path) -> None:
    ...
    result = CliRunner().invoke(
        cli, ["eval", "coverage", "--suite", str(suite), "--format", "json"]
    )
    assert result.exit_code == 0, result.output
...
def test_coverage_out_writes_file(monkeypatch, tmp_path) -> None:
    ...
    result = CliRunner().invoke(
        cli, ["eval", "coverage", "--suite", str(suite), "--out", str(out)]
    )
    assert result.exit_code == 0, result.output
    assert out.exists()
...
def test_coverage_unknown_suite_errors_cleanly(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("ATLAS_REPOSITORY_BASE_PATH", str(tmp_path))
    result = CliRunner().invoke(
        cli, ["eval", "coverage", "--suite", "not-a-real-file.json"]
    )
...
# --- eval validate-cases -------------------------------------------------------------
def test_validate_cases_passes_when_no_case_has_provenance(
    monkeypatch, tmp_path
) -> None:
    def _exploding(*a, **k):
        raise AssertionError(
            "build_llm_client must never be called by `eval validate-cases`"
        )
