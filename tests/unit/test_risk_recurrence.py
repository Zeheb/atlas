...
def test_ordering_count_desc_then_recency_desc_then_text_asc() -> None:
    prof = _profile([
        ...
        _risk("2020-03-31", "Alpha risk"), _risk("2022-03-31", "Alpha risk"), _risk("2024-03-31", "Alpha risk"),
        # "beta risk": 2 occurrences, most recent 2025 (more recent than alpha, but fewer occurrences)
        _risk("2023-03-31", "Beta risk"), _risk("2025-03-31", "Beta risk"),
        # "gamma risk": 2 occurrences, most recent 2021 (least recent)
        _risk("2019-03-31", "Gamma risk"), _risk("2021-03-31", "Gamma risk"),
    ])
    rows = risk_recurrence(prof).sections[0].rows
...
def test_text_tiebreak_when_count_and_recency_equal() -> None:
    prof = _profile([
        _risk("2022-03-31", "Zeta risk"), _risk("2023-03-31", "Zeta risk"),
        _risk("2022-03-31", "Alpha risk"), _risk("2023-03-31", "Alpha risk"),
    ])
    rows = risk_recurrence(prof).sections[0].rows
    texts = [r[2].lower() for r in rows]
...
def test_query_registered_and_dispatchable() -> None:
    ...
    prof = _profile([_risk("2022-03-31", "Market risk"), _risk("2023-03-31", "Market risk")])
