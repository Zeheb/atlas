...
def test_plan_none_is_byte_identical_to_m15(tmp_path: Path) -> None:
    ...
    ctx_explicit_none_plan = build_context(
        _profile(), SUBJECT, kb=kb, question=_QUESTION, plan=None
    )
...
def test_matching_question_and_plan_is_accepted(tmp_path: Path) -> None:
    kb = _kb_with(tmp_path, "ev-1", _CONTENT)
    build_context(
        _profile(), SUBJECT, kb=kb, question=_QUESTION, plan=_plan()
    )  # must not raise
...
def test_plan_doc_type_boost_changes_which_passage_is_kept(tmp_path: Path) -> None:
    ...
    unplanned_ids = {
        ref.evidence_id
        for c in ctx_unplanned.claims
        if c.statement.startswith("Source passage:")
...
def test_plan_never_introduces_an_evidence_id_outside_profile_claims(
    tmp_path: Path,
) -> None:
    kb = _kb_with(tmp_path, "ev-1", _CONTENT)
    plan = _plan(top_k=5)
