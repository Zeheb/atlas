...
def _case(recalled_view: RecalledViewFixture | None) -> EvalCase:
    return EvalCase(
        id="t-thesis",
        category="A",
        question="Does anything contradict my thesis?",
        subject="TCS",
        expected_behavior="answer",
        rubric="check",
        requires=("thesis",),
        recalled_view=recalled_view,
    )


def test_fixture_is_projected_into_context_when_cap_thesis_is_active(
    tmp_path: Path,
) -> None:
    _seed(tmp_path)
    settings = Settings(_env_file=None, repository_base_path=tmp_path)
    runner = LiveReasoningRunner(
        settings, None, capabilities=frozenset({CAP_THESIS}), retrieval_only=True
    )
...
def test_fixture_is_not_injected_when_cap_thesis_is_absent(tmp_path: Path) -> None:
    ...
    settings = Settings(_env_file=None, repository_base_path=tmp_path)
    runner = LiveReasoningRunner(
        settings, None, capabilities=frozenset(), retrieval_only=True
    )
    outcome = runner.run(_case(_fixture()))
