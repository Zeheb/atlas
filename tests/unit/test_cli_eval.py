...
def test_eval_run_writes_report_and_prints_summary(monkeypatch, tmp_path) -> None:
    ...
    assert (
        report["cache_hits"] == 0 and report["cache_misses"] == 2
    )  # reasoning + judge
...
def test_eval_run_judge_sample_limits_judge_calls(monkeypatch, tmp_path) -> None:
        ...
        json.dumps(
            [
                ...
                },
                {
                    "id": "t02",
                    "category": "A",
                    "question": "How stable is revenue?",
                    "subject": "TCS",
                    "expected_behavior": "answer",
                    "rubric": "synthesize",
                },
    ...
    expected_sampled = min(
        by_id, key=lambda cid: hashlib.sha256(cid.encode()).hexdigest()
    )
    ...
    assert (
        by_id[unjudged[0]]["correctness_pass"] is True
    )  # deterministic scoring unaffected


def test_eval_run_suite_preset_core_runs_a_small_bundled_subset(
    ...
    assert (
        1 < len(report["results"]) < len(load_cases())
    )  # a curated subset, not the full suite
        ...
        return {
            ...
            "results": [
                ...
                },
                {
                    ...
                    "refused": None,
                    "correctness_pass": True if second_status == "active" else None,
                    "correctness_reasons": [],
                    "grounding_pass": True if second_status == "active" else None,
                    "grounding_reasons": [],
...
def test_eval_run_retrieval_only_builds_no_llm_client_at_all(
    monkeypatch, tmp_path
) -> None:
    def _exploding_build_llm_client(settings, *, role):
        raise AssertionError(
            f"build_llm_client(role={role!r}) must never be called in --retrieval-only mode"
        )

    monkeypatch.setenv("ATLAS_REPOSITORY_BASE_PATH", str(tmp_path))
    monkeypatch.setattr(
        "atlas.reasoning.llm.build_llm_client", _exploding_build_llm_client
    )
...
def test_eval_run_retrieval_only_and_with_answers_are_mutually_exclusive(
    monkeypatch, tmp_path
) -> None:
    monkeypatch.setenv("ATLAS_REPOSITORY_BASE_PATH", str(tmp_path))
    result = CliRunner().invoke(
...
def test_eval_run_with_strategy_populates_retrieval_metrics(
    monkeypatch, tmp_path
) -> None:
    monkeypatch.setenv("ATLAS_REPOSITORY_BASE_PATH", str(tmp_path))
    monkeypatch.setenv("ATLAS_ANTHROPIC_API_KEY", "sk-test")
...
def test_eval_compare_retrieval_prints_recommendation_and_writes_full_json(
    tmp_path,
) -> None:
            ...
            "results": [
                {
                    ...
                    "retrieval_metrics": {
                        ...
                        "docs_searched": 2,
                        "selected": [["ev-1", 0, 100]],
                        "doc_type_counts": [["annual_report", 1]],
    ...
    assert any(
        v in result.output for v in ("SAFE_TO_ENABLE", "NOT_READY", "INSUFFICIENT_DATA")
    )
    assert out.exists()
