...
def make_profile(company_id: str = "ACME") -> CompanyProfile:
    ...
    esg = ESGTimeSeries(
        snapshots=[
            ESGSnapshot(
                period="2025-03-31",
                facts={
                    FactKind.ESG_WORKFORCE_HEADCOUNT: 10_000.0,
                    FactKind.ESG_WORKFORCE_ATTRITION_PCT: 12.0,
                },
                sources=["ev-esg-2025"],
            ),
            ESGSnapshot(
                period="2026-03-31",
                facts={
                    FactKind.ESG_WORKFORCE_HEADCOUNT: 10_500.0,
                    FactKind.ESG_WORKFORCE_ATTRITION_PCT: 14.0,
                },
                sources=["ev-esg-2026"],
            ),
        ]
    ...
    ownership = OwnershipTimeSeries(
        snapshots=[
            OwnershipSnapshot(
                period="2026-03-31",
                facts={
                    FactKind.OWNERSHIP_PROMOTER_PCT: 55.0,
                    FactKind.OWNERSHIP_FPI_PCT: 20.0,
                },
                sources=["ev-own-2026"],
            ),
        ]
    ...
    capital_events = CapitalEventLedger(
        dividends=[
            DividendEvent(
                source_date=_dt("2026-04-01"),
                per_share=5.0,
                dividend_type="final",
                evidence_id="ev-div-1",
            )
        ],
        buybacks=[
            BuybackEvent(
                source_date=_dt("2025-12-01"),
        ...
        acquisitions=[
            AcquisitionEvent(
                source_date=_dt("2025-11-01"),
                target_name="Widget Co",
                stake_pct=100.0,
                expected_completion="2026-09-30",
                evidence_id="ev-acq-1",
            )
        ],
        fundraises=[
            FundraisingEvent(
                source_date=_dt("2026-03-01"),
                fundraise_type="NCD",
                amount=2_000.0,
                evidence_id="ev-fund-1",
            )
        ],
    )
    ...
    credit_history = CreditHistory(
        debt_ratings=[
            CreditRatingEntry(
                source_date=_dt("2026-01-01"),
                agency="CRISIL",
                rating="AAA",
                action="reaffirmed",
                evidence_id="ev-rate-1",
            )
        ],
    )
    ...
    strategy = StrategyProfile(
        entries=[
            StrategyEntry(
                source_date=_dt("2025-06-01"),
                kind="guidance",
                text="We target an aspirational margin band of 20-22% over time.",
                evidence_id="ev-strat-1",
            ),
            StrategyEntry(
                source_date=_dt("2026-04-01"),
                kind="aspiration",
                text="We aim to be the market leader.",
                evidence_id="ev-strat-2",
            ),
            StrategyEntry(
                source_date=_dt("2026-06-01"),
                kind="guidance",
                text="We reiterate our margin band of 20-22% as the medium-term target.",
                evidence_id="ev-strat-3",
            ),
        ]
    )
    ...
    governance = GovernanceProfile(
        resolutions=[
            AGMResolution(
                source_date=_dt("2026-07-01"),
                period="2026-06-30",
                title="Re-appointment of auditor",
                resolution_type="ordinary",
                outcome="passed",
                evidence_id="ev-agm-1",
            )
        ],
        director_changes=[
            DirectorChange(
                source_date=_dt("2026-02-01"),
                change_type="appointment",
                name="Jane Doe",
                role="Independent Director",
                evidence_id="ev-dir-1",
            )
        ],
        ...
        risk_factors=[
            RiskEntry(
                period="2025-03-31",
                text="Currency fluctuations may adversely impact reported revenue and margins over time.",
                evidence_id="ev-risk-1",
            ),
            RiskEntry(
                period="2026-03-31",
                text="Currency fluctuations may adversely impact reported revenue and margins over time.",
                evidence_id="ev-risk-2",
            ),
