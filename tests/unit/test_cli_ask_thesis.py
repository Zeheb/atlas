...
def _remember(base: Path, ticker: str = "TCS", evidence_id: str = VIEW_EVIDENCE) -> Thesis:
    ...
    claim = Claim(
        subject_ref=subject, statement="Margins were declining.",
    ...
    result = ReasoningResult(
        ...
        findings=(Finding(
            statement="Margins were declining.", assertability="judgment",
            confidence="medium", supporting_claims=(claim,),
        ),),
        overall_confidence="medium", citations=frozenset({evidence_id}), refused=False,
    )
    fingerprint = "fp-1"
    thesis = Thesis(
        question="Should I invest in TCS?", subjects=(ticker,),
        run_fingerprint=fingerprint,
        view_id=compute_view_id(fingerprint, "Should I invest in TCS?"),
        as_of="2026-01-01T00:00:00+00:00", result=result,
    )
    ...
    def complete(self, *, system: str, user: str) -> str:
        ...
        return json.dumps({
            ...
            "findings": [{
                ...
                "confidence": "medium", "supporting_evidence_ids": [CURRENT_EVIDENCE],
    ...
    def complete(self, *, system: str, user: str) -> str:
        return json.dumps({
            ...
            "findings": [{
                ...
                "supporting_evidence_ids": [VIEW_EVIDENCE], "known_unknowns": [],
            }],
        })
...
def test_valid_evidence_ids_never_include_the_recalled_views_evidence(
    ...
    CliRunner().invoke(cli, ["ask", "TCS", "What about margins?", "--thesis", thesis.view_id])

    prompt = client.user_prompts[0]
    valid_line = next(l for l in prompt.splitlines() if l.startswith("VALID EVIDENCE IDS"))
...
def test_unknown_view_id_exits_one(monkeypatch, tmp_path) -> None:
    ...
    result = CliRunner().invoke(
        cli, ["ask", "TCS", "What about margins?", "--thesis", "no-such-view"],
    )
...
def test_no_store_at_all_exits_one(monkeypatch, tmp_path) -> None:
    ...
    result = CliRunner().invoke(
        cli, ["ask", "TCS", "What about margins?", "--thesis", "anything"],
    )
    assert result.exit_code == 1
    assert "No remembered view" in result.output


def test_incompatible_store_exits_one_without_a_traceback(monkeypatch, tmp_path) -> None:
    _env(monkeypatch, tmp_path, _CapturingFake())
    _seed_profile(tmp_path)
