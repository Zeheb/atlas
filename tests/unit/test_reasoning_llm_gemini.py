...
class _FakeGenAIModels:
    ...
    ):  # noqa: ANN001, ANN201 - test double
...
class _FakeGenAIClient:
    ...
    def __init__(self, *, api_key: str | None = None, **kwargs) -> None:  # noqa: ANN003
        ...
        self.models = _FakeGenAIModels(
            response_text="Operating margin has held near 24%."
        )
...
def test_from_settings_builds_with_key(monkeypatch) -> None:
    ...
    client = GeminiClient.from_settings(
        Settings(_env_file=None), provider="google_ai_studio"
    )
    assert isinstance(client, LLMClient)
...
def test_from_settings_default_max_tokens_and_temperature_match_anthropic_parity(
    monkeypatch,
) -> None:
...
def test_from_settings_vertex_ai_raises_not_implemented_regardless_of_key(
    monkeypatch,
) -> None:
    monkeypatch.setenv(
        "ATLAS_GEMINI_API_KEY", "g-test-key"
    )  # key present is irrelevant
...
def test_from_settings_vertex_ai_raises_before_checking_for_a_key() -> None:
    ...
    with pytest.raises(
        NotImplementedError
    ):  # NotImplementedError, not MissingAPIKeyError
...
def test_complete_calls_sdk_with_correct_shape() -> None:
    client = GeminiClient(
        api_key="k", model="gemini-2.5-pro", max_tokens=2048, temperature=0.3
    )
...
def test_complete_returns_empty_string_when_response_text_is_none(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda **kw: type(
            "C",
            (),
            {"models": _FakeGenAIModels(response_text=None)},
        )(),
    )
