...
class TestSingleSourceIsNoOp:
    ...
    def test_single_source_multiple_documents_all_pass_through(self):
        docs = [
            _ev("bse-1", EvidenceSource.BSE, date=datetime(2026, 4, 9, tzinfo=timezone.utc)),
            _ev("bse-2", EvidenceSource.BSE, date=datetime(2026, 1, 9, tzinfo=timezone.utc)),
        ...
        assert set(e.evidence_id for e in merged.evidence) == {"bse-1", "bse-2", "bse-3"}

    def test_single_source_warnings_preserved(self):
        warning = DiscoveryWarning(source=EvidenceSource.BSE, code="test", message="x")
        bse_result = DiscoveryResult(evidence=[_ev("bse-1", EvidenceSource.BSE)], warnings=[warning])
        merged = resolve_multi_source([bse_result])
...
class TestDeduplication:
    def test_same_filing_from_two_exchanges_deduplicated_to_one(self):
        ...
        bse = DiscoveryResult(evidence=[_ev("bse-1", EvidenceSource.BSE, date=same_date)])
        nse = DiscoveryResult(evidence=[_ev("nse-1", EvidenceSource.NSE, date=same_date)])
        merged = resolve_multi_source([bse, nse])
        assert len(merged.evidence) == 1

    def test_larger_file_size_preferred(self):
        same_date = datetime(2026, 4, 9, tzinfo=timezone.utc)
        bse = DiscoveryResult(evidence=[_ev("bse-1", EvidenceSource.BSE, date=same_date, file_size_bytes=100)])
        nse = DiscoveryResult(evidence=[_ev("nse-1", EvidenceSource.NSE, date=same_date, file_size_bytes=500)])
        merged = resolve_multi_source([bse, nse])
    ...
    def test_equal_size_falls_back_to_bse(self):
        same_date = datetime(2026, 4, 9, tzinfo=timezone.utc)
        bse = DiscoveryResult(evidence=[_ev("bse-1", EvidenceSource.BSE, date=same_date, file_size_bytes=100)])
        nse = DiscoveryResult(evidence=[_ev("nse-1", EvidenceSource.NSE, date=same_date, file_size_bytes=100)])
        merged = resolve_multi_source([bse, nse])
        assert merged.evidence[0].source == EvidenceSource.BSE

    def test_different_kinds_same_day_not_deduplicated(self):
        same_date = datetime(2026, 4, 9, tzinfo=timezone.utc)
        bse = DiscoveryResult(evidence=[_ev("bse-1", EvidenceSource.BSE, kind=EvidenceKind.FINANCIAL_RESULTS, date=same_date)])
        nse = DiscoveryResult(evidence=[_ev("nse-1", EvidenceSource.NSE, kind=EvidenceKind.BOARD_OUTCOME, date=same_date)])
        merged = resolve_multi_source([bse, nse])
        assert len(merged.evidence) == 2

    def test_different_dates_not_deduplicated(self):
        bse = DiscoveryResult(evidence=[_ev("bse-1", EvidenceSource.BSE, date=datetime(2026, 4, 9, tzinfo=timezone.utc))])
        nse = DiscoveryResult(evidence=[_ev("nse-1", EvidenceSource.NSE, date=datetime(2026, 1, 9, tzinfo=timezone.utc))])
        merged = resolve_multi_source([bse, nse])
        assert len(merged.evidence) == 2
    ...
    def test_dedup_emits_provenance_warning(self):
        same_date = datetime(2026, 4, 9, tzinfo=timezone.utc)
        bse = DiscoveryResult(evidence=[_ev("bse-1", EvidenceSource.BSE, date=same_date, file_size_bytes=500)])
        nse = DiscoveryResult(evidence=[_ev("nse-1", EvidenceSource.NSE, date=same_date, file_size_bytes=100)])
        merged = resolve_multi_source([bse, nse])
    ...
    def test_suppressed_evidence_id_recoverable_from_warning_not_lost(self):
        ...
        merged = resolve_multi_source([bse, nse])
        all_suppressed_ids = [
            eid for w in merged.warnings if w.code == "duplicate_across_exchanges"
    ...
    def test_three_way_duplicate_keeps_exactly_one(self):
        ...
        bse = DiscoveryResult(evidence=[_ev("bse-1", ES.BSE, date=same_date, file_size_bytes=100)])
        nse = DiscoveryResult(evidence=[_ev("nse-1", ES.NSE, date=same_date, file_size_bytes=200)])
        mca = DiscoveryResult(evidence=[_ev("mca-1", ES.MCA, date=same_date, file_size_bytes=50)])
