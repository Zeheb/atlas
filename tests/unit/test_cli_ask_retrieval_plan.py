...
def test_retrieval_plan_flag_still_merges_passage_into_prompt(
...
def test_retrieval_plan_without_question_retrieval_is_a_silent_no_op(
    monkeypatch, tmp_path: Path
) -> None:
    ...
    result, fake = _run_and_capture_fake(
        monkeypatch,
        tmp_path,
        ["ask", "TCS", _QUESTION, "--retrieval-plan"],
    )
    assert result.exit_code == 0, result.output
    _system, user_prompt = fake.calls[0]
...
def test_explain_plan_without_retrieval_plan_prints_nothing(
    monkeypatch, tmp_path: Path
) -> None:
    result, _fake = _run_and_capture_fake(
        monkeypatch,
