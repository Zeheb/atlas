...
def _claim(statement: str, ev: str) -> Claim:
    return Claim(
        ...
        confidence="medium",
        evidence=(EvidenceReference(evidence_id=ev),),
    )
...
def test_question_mismatch_refuses_comparison() -> None:
    older = _thesis(
        "Is margin durable?", "fp1", "2024-01-01", "medium", [("X", "medium")]
    )
    newer = _thesis(
        "Is the balance sheet safe?", "fp2", "2025-01-01", "medium", [("Y", "medium")]
    )
...
def test_question_normalized_for_matching_too() -> None:
    ...
    newer = _thesis(
        "is margin durable", "fp2", "2025-01-01", "medium", [("X", "medium")]
    )
    assert (
        diff_theses(older, newer) is not None
    )  # same question, punctuation/case differ
...
def test_diff_directionality_inverts_on_reversed_arguments() -> None:
    ...
    t2 = _thesis(
        "Is margin durable?", "fp2", "2025-01-01", "high", [("New finding.", "high")]
    )
