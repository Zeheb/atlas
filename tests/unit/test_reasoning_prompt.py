...
def test_no_excerpt_renders_terse_fact_only_m0_equivalent() -> None:
    claim = Claim(
        subject_ref=SUBJECT, statement="operating margin = 24.2", assertability="fact",
...
def test_hydrated_excerpt_is_rendered_alongside_fact() -> None:
    ref = EvidenceReference(
        evidence_id="ev-1", excerpt="Operating margin stood at 24.2% in FY26.",
    )
    claim = Claim(
        subject_ref=SUBJECT, statement="operating margin = 24.2", assertability="fact",
...
def test_duplicate_excerpts_across_refs_render_once() -> None:
    ...
    claim = Claim(
        subject_ref=SUBJECT, statement="operating margin = 24.2", assertability="fact",
        confidence="high", evidence=refs,
    )
    context = GroundingContext(
        subject_ref=SUBJECT, claims=[claim], evidence_index=frozenset({"ev-1", "ev-2"}),
    )
    prompt = build_user_prompt(_question(), context)
