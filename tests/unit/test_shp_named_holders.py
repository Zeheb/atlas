...
def test_public_categories_map_to_kind() -> None:
    ...
    assert _public_holder_class("OtherInstitutions") == ("other_institution", "organization")
    assert _public_holder_class("OtherNonInstitutions") == ("other_non_institution", "organization")
...
def _fmap() -> dict[tuple[str, str], str]:
    return {
        ...
        ("NameOfTheShareholder", "D_InsuranceCompanies_Context1"): "Life Insurance Corporation of India",
        ("NameOfTheShareholder", "D_OthersIndianShareholders_Context15"): "TATA SONS PRIVATE LIMITED",
        ("NameOfTheShareholder", "D_ResidentIndividualShareholdersHoldingNominalShareCapitalInExcessOfRsTwoLakh_Context2"): "Ashish Kacholia",
...
def test_kind_and_category_from_axis() -> None:
    ms = {m.entity.canonical_name: m for m in _extract_named_public_holders(_fmap(), EntityResolver())}
...
def _shp_result(evidence_id: str, mentions: list[EntityMention]) -> AnalysisResult:
    return AnalysisResult(
        evidence_id=evidence_id, kind="shareholding_pattern", analyzer_version="1.1",
        confidence="high", source_date=datetime(2026, 3, 31, tzinfo=timezone.utc),
        entities=mentions,
    )
...
def _mention(name: str, category: str, kind: str) -> EntityMention:
    return EntityMention(
        entity=Entity(entity_id=f"{kind}:{name.lower().replace(' ', '-')}", kind=kind, canonical_name=name),
        role=category, affiliation=None,
    )


def test_builder_ingests_named_shareholders() -> None:
    result = _shp_result("bse-shp-1", [
...
def test_named_shareholders_survive_store_round_trip(tmp_path) -> None:
    result = _shp_result("bse-shp-2", [_mention("Life Insurance Corporation of India", "insurance", "organization")])
    profile = build_profile("ACME", [result])
