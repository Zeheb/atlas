...
def test_label_kind_matching_real_kind_passes(tmp_path: Path) -> None:
    ...
    case = _Case(
        ...
        retrieval_label=RetrievalLabel(
            relevant_evidence_ids=("ev-1",), relevant_kinds=("brsr",)
        ),
    )
    report = validate_cases([case], tmp_path)
    assert report.passed
...
def test_label_kind_mismatch_detected(tmp_path: Path) -> None:
    ...
    case = _Case(
        ...
        provenance=_provenance(),
        retrieval_label=RetrievalLabel(
            relevant_evidence_ids=("ev-1",), relevant_kinds=("annual_report",)
        ),
    )
    report = validate_cases([case], tmp_path)
    assert not report.passed
...
def test_negative_case_with_no_overlap_passes(tmp_path: Path) -> None:
    ...
    case = _Case(
        id="c1",
        subject="TCS",
        question="xyzzyplugh wibbleflorp nonexistent gibberish term",
        provenance=CaseProvenance(
            origin="corpus_validated_negative",
            verification_method="ran retrieval, nothing matched",
            verified_at="2026-07-21",
            verified_by="z",
        ),
    )
...
def test_negative_case_with_real_overlap_fails(tmp_path: Path) -> None:
    ...
    case = _Case(
        id="c1",
        subject="TCS",
        question="What was the operating margin 24.2 percent in FY26?",
        provenance=CaseProvenance(
            origin="corpus_validated_negative",
            verification_method="claimed absent",
            verified_at="2026-07-21",
            verified_by="z",
        ),
    )
...
def test_kb_loaded_once_per_subject_not_per_case(tmp_path: Path) -> None:
    _seed(tmp_path)
    cases = [
        _Case(id=f"c{i}", subject="TCS", question="q", provenance=_provenance())
        for i in range(5)
    ]
...
def test_one_bad_case_does_not_suppress_others(tmp_path: Path) -> None:
    ...
    cases = [
        ...
        _Case(
            ...
            subject="TCS",
            question="q",
            provenance=_provenance(supporting_evidence_ids=("ghost",)),
        ),
    ]
    report = validate_cases(cases, tmp_path)
