...
def _entry(
    evidence_id: str, kind: str, source_date: str, title: str = "x"
) -> CatalogEntry:
    return CatalogEntry(
...
def _fsnap(
    period, facts, period_type="quarterly", basis="consolidated", sources=None
) -> FinancialSnapshot:
...
class TestResolvePeriod:
    ...
    def test_found_in_financial_snapshot(self):
        ...
        p.financial = FinancialTimeSeries(
            snapshots=[_fsnap("2026-03-31", {}, sources=["e1"])]
        )
    ...
    def test_esg_snapshot_match(self):
        p = CompanyProfile(company_id="TCS")
        p.esg = ESGTimeSeries(
            snapshots=[ESGSnapshot(period="2026-03-31", facts={}, sources=["e1"])]
        )
...
class TestResolveAgency:
    ...
    def test_checks_esg_ratings_too(self):
        ...
        p.credit_history = CreditHistory(
            esg_ratings=[
                CreditRatingEntry(
                    source_date=_DT, agency="NSE Sustainability", evidence_id="e2"
                ),
            ]
        )
...
class TestDisplayNameTemplates:
    def _profile_with(
        self, evidence_id: str, period: str, period_type: str = "quarterly"
    ) -> CompanyProfile:
        p = CompanyProfile(company_id="TCS")
        p.financial = FinancialTimeSeries(
            snapshots=[
                _fsnap(period, {}, period_type=period_type, sources=[evidence_id])
            ]
        )
        return p
...
class TestCitationFormats:
    def _tcs_transcript_citation(self):
        ...
        p = CompanyProfile(company_id="TCS")
        p.financial = FinancialTimeSeries(
            snapshots=[_fsnap("2026-03-31", {}, period_type="annual", sources=["e1"])]
        )
    ...
    def test_citation_full_without_section_or_page(self):
        ...
        assert (
            c.citation_full
            == "TCS Q4 FY2026 Earnings Call Transcript\nPublished: Apr 2026"
        )
